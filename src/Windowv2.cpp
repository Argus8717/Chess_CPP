...
void moveSpriteToSquare(sf::Sprite& sprite, const std::string& squareName)
{
    for (const auto& namedPos : namedPositions)
    {
        if (namedPos.name == squareName)
        {
            sprite.setPosition(namedPos.position);
            return;
        }
    }
    std::cerr << "Invalid square name: " << squareName << std::endl;
}

void moveSpriteToBoundSquare(sf::Sprite& sprite, const std::string& squareName)
{
    for (const auto& namedBound : namedBoundsPositions)
    {
        if (namedBound.name == squareName + "_bounds")
        {
            sprite.setPosition(namedBound.position);
            return;
        }
    }
    std::cerr << "Invalid square name for bounds: " << squareName << std::endl;
}
...
void Window::pollEvents()
    {
        ...
        if (window->hasFocus())
        {
            if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
            {
                ...
                for (size_t i = 0; i < namedPositions.size(); ++i)
                {
                    ...
                    {
                        ::moveSpriteToTarget(pieceHighlightSprite, topLeft.x, topLeft.y);
                        std::cout << "Position: " << topLeft.x << ", " << topLeft.y << std::endl;
                        break;
                    }
                }
            }
            ...
            {
                if (piecesOnBoard[i]->pieceHighlightSprite())
                {
                    {
                        //Check if there is a piece on highlighted square. If true, move piece where you right click
                        sf::Vector2i mousePosition = sf::Mouse::getPosition(*window);
                        ...
                        for (size_t i = 0; i < namedPositions.size(); ++i)
                        {
                            ...
                            if (mousePosF.x >= topLeft.x && mousePosF.x <= bottomRight.x && mousePosF.y >= topLeft.y &&
                                mousePosF.y <= bottomRight.y)
                            {
                                if (piecesOnBoard[i] != nullptr)
                                {
                                    std::cout << "Already piece there" << std::endl;
                                } else
                                {
                                    movePieceToPosition(whitePawn4Sprite, topLeft);
                                    std::cout << "Move to Cell: " << topLeft.x << ", " << topLeft.y << std::endl;
                                }
                                break;
                            }
                        }
        ...
        while (const std::optional event = window->pollEvent())
        {
            ...
            } else if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>())
            {
                ...
                {
                    pieceHighlightSprite.setPosition(sf::Vector2f(546, 132));
                    ::moveSpriteToTarget(pieceHighlightSprite, 546, 132);
                    whitePawn4Sprite.setPosition(sf::Vector2f(546, 132));
                    ::moveSpriteToTarget(whitePawn4Sprite, 546, 132);
                }
            }
        }*/
    }
...
//Load highlight
    ...
    if (!pieceHighlightTexture.loadFromFile("PieceHighlight.png"))
        return;
    ...
    pieceHighlightSprite.setPosition(sf::Vector2f(546.f, 510.f));
